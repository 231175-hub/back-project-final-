...
public class DataInitializer {
	...
	CommandLineRunner initDatabase(
		...
		return args -> {
			runDatabaseMigrations(jdbcTemplate);
			syncLegacyUserPasswords(jdbcTemplate, passwordEncoder);
			initializeRoles(repositoryRole);
			initializeAdminUser(repositoryRole, repositoryUser, passwordEncoder);
		};
	}
	...
	private void initializeRoles(RepositoryRole repositoryRole) {
	...
	private void initializeAdminUser(RepositoryRole repositoryRole, RepositoryUser repositoryUser, PasswordEncoder passwordEncoder) {
		...
		try {
			...
			if (usersWithEmail.isEmpty()) {
				log.info("Seeder: Creando Administrador Maestro ({})...", masterEmail);
				...
				log.info("=========================================");
				log.info("ADMINISTRADOR MAESTRO CREADO EXITOSAMENTE");
				log.info("Usuario: {}", masterEmail);
				log.info("Contraseña: {}", masterPassword);
				log.info("=========================================");
			} else {
				...
				log.info("Seeder: El Administrador Maestro {} fue actualizado con la clave maestra.", masterEmail);
			}
		} catch (Throwable e) {
			log.error("Error procesando Administrador Maestro: {}", e.getMessage(), e);
		}
	}
}
