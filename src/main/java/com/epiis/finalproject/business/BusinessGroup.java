...
public class BusinessGroup {
	...
	public BusinessGroup(RepositoryGroup repositoryGroup, RepositoryGroupStudent repositoryGroupStudent,
			RepositoryCourseEnrollment repositoryCourseEnrollment, RepositoryAcademicperiod repositoryAcademicperiod,
			RepositoryCourse repositoryCourse, RepositoryUnits repositoryUnits) {
	...
	public ResponseGroupStudentInsert createAndAssignGroups(RequestGroupAssignment request) {
		...
		Optional<EntityAcademicPeriod> optionalActivePeriod = repositoryAcademicperiod
				.findByStatus(EnumAcademicPeriod.ACTIVE.toString());
		...
		List<EntityCourseEnrollment> unassignedStudents = repositoryCourseEnrollment
				.findUnassignedStudents(request.getIdCourse(), activePeriod.getIdPeriod());
		...
		response.getListMessage().add(
				"Se crearon " + numberOfGroups + " grupos y se asignaron " + totalStudents + " estudiantes al azar.");
		return response;
	}

	public List<ResponseUnassignedStudent> getUnassignedStudents(String idCourse) {
		EntityAcademicPeriod activePeriod = repositoryAcademicperiod.findByStatus(EnumAcademicPeriod.ACTIVE.toString())
				.orElseThrow(() -> new RuntimeException("Error: No hay un periodo académico activo actualmente"));
